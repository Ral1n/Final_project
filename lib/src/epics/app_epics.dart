...
class AppEpics extends EpicClass<AppState> {
  ...
  Stream<dynamic> call(Stream<dynamic> actions, EpicStore<AppState> store) {
    return combineEpics(<Epic<AppState>>[
      TypedEpic<AppState, LoadPhotosStart>(_loadPhotosStart).call,
  ...
  Stream<AppAction> _loadPhotosStart(Stream<LoadPhotosStart> actions, EpicStore<AppState> store) {
    return actions.debounceTime(const Duration(milliseconds: 1000)).switchMap((LoadPhotosStart action) {
      return Stream<void>.value(null)
          .asyncMap((_) {
            return api.loadPhotos(store.state.page, query: store.state.query, color: store.state.color);
          })
          .map((List<Photo> photos) => LoadPhotos.successful(photos))
          .onErrorReturnWith((Object error, StackTrace stackTrace) => LoadPhotos.error(error, stackTrace));
    });
  }
